...
void TomVMDLLAdapter::ReadArray(
    int dataIndex,
    int *dest,
    int dimensionCount,
    int *dimensions) {
...
void TomVMDLLAdapter::WriteArray(
    int dataIndex,
    int *src,
    int dimensionCount,
    int *dimensions) {
...
void DLLFUNC TomVMDLLAdapter::GetFloatArrayParam(
    ...
    // This is a bit of a hack, and relies on the fact that floats and ints are
    // simply 32bit chunks of data, and that we are simply copying the values,
    // without any need to interpret them.

    // If this were not the case (e.g. on different hardware) we would need to
    // rewrite this.
    ReadArray(
        vm.GetIntParam(index),
        (int *) array,
...
void DLLFUNC TomVMDLLAdapter::SetIntArrayResult(
    ...
    int dataIndex = CreateTempArray(VTP_INT, dimensions, &dimension0Size);

    // Fill it with data
    WriteArray(dataIndex, array, dimensions, &dimension0Size);
...
void DLLFUNC TomVMDLLAdapter::SetFloatArrayResult(
    ...
    // Fill it with data
    WriteArray(dataIndex, (int *) array, dimensions, &dimension0Size);
...
void TomVMDLLAdapter::CValueFromBasicValue(
    ...
    switch (type.baseType) {
        ...
        case DLL_BASIC4GL_EXT_WORD:
            ...
            *((__int16*)cData) = value.IntVal();
            cData += sizeof(__int16);
            break;
        ...
        case DLL_BASIC4GL_EXT_INT64:
            ...
            *((__int64*)cData) = value.IntVal();
            cData += sizeof(__int64);
            break;
...
void TomVMDLLAdapter::BasicArrayFromCArray(
    ...
    // Get array size
    int count = vm.Data().Data()[basicDataIndex].IntVal();
    int size = vm.Data().Data()[basicDataIndex + 1].IntVal();
    int arrayStart = basicDataIndex + 2;
    ...
    // Advance data index
    basicDataIndex = arrayStart + count * size;
}
...
void DLLFUNC TomVMDLLAdapter::ModTypeArray(
    ...
    int *dimensionSize = &dimension0Size;
...
void DLLFUNC TomVMDLLAdapter::GetParam(
    ...
    if (currentType.arrayLevel == 0 &&
        ...
        CValueFromBasicValue(currentType, data, vm.GetParam(index));
    }
    else {
        ...
        // Convert data
        CDataFromBasicData(derefType, data, dataIndex);
    }
}
...
void DLLFUNC TomVMDLLAdapter::SetParam(
    ...
    if (currentType.arrayLevel == 0 &&
            currentType.pointerLevel == 0 &&
            currentType.baseType < 0) {
        BasicValueFromCValue(currentType, vm.GetParam(index), data);
    }
    else {
        ...
        // Convert data
        BasicDataFromCData(derefType, dataIndex, data);
    }
}
...
void DLLFUNC TomVMDLLAdapter::SetReturnValue(
    ...
    if (currentType.arrayLevel == 0 &&
        ...
        if (currentType.baseType == DLL_BASIC4GL_EXT_STRING)
            ...
            BasicValueFromCValue(currentType, vm.Reg(), data);
    }
    else {
