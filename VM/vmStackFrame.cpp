...
std::string vmUserFuncPrototype::GetLocalVarName(int index) {
...
bool vmUserFuncPrototype::Matches(vmUserFuncPrototype& func) {
    ...
    for (   map<string,int>::iterator i = localVarIndex.begin();
        ...
        if (i->second < paramCount) {
            ...
            map<string,int>::iterator funcI = func.localVarIndex.find(i->first);
...
void vmUserFuncPrototype::StreamOut(ostream& stream) {
...
#ifdef VM_STATE_STREAMING
void vmUserFunc::StreamOut(std::ostream& stream) {
...
// vmRuntimeFunction
#ifdef VM_STATE_STREAMING
void vmRuntimeFunction::StreamOut (std::ostream& stream) {
