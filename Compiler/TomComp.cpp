...
TomBasicCompiler::TomBasicCompiler (TomVM& vm, PluginDLLManager& plugins, bool caseSensitive)
...
bool TomBasicCompiler::CompileFunction (bool needResult) {
    ...
    m_plugins.FindFunctions(m_token.m_text, functions, functionCount, TC_MAXOVERLOADEDFUNCTIONS);
    ...
    bool brackets = false;
    bool isSub = true;
    bool isNoParamFunction = true;
    for (int i = 0; i < functionCount; i++) {
        compFuncSpec* spec = functions[i].m_spec;
        if (spec->m_isFunction)
            isSub = false;
        if (!spec->m_isFunction || spec->m_paramTypes.Params().size() > 0)
            isNoParamFunction = false;
        if (spec->m_brackets)
            brackets = true;
    }

    // In traditional syntax, we don't require brackets for subs or functions
    // with no parameters
    if ((m_syntax == LS_TRADITIONAL || m_syntax == LS_TRADITIONAL_SUFFIX) && (isSub || isNoParamFunction))
        brackets = false;
    ...
    while (functionCount > 0 && m_token.m_text != ")" && !AtSeparatorOrSpecial () && !isNoParamFunction) {
...
bool TomBasicCompiler::CompileUserFunctionCall(bool mustReturnValue, bool isRuntimeFunc) {
    ...
    bool brackets = true;
    if (m_syntax == LS_TRADITIONAL || m_syntax == LS_TRADITIONAL_SUFFIX)
