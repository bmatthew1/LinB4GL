...
void glBasicSprite::SetDefaults () {
    m_zOrder        = 0;
    m_list          = NULL;
    m_prev          = NULL;
    m_next          = NULL;
...
glSpriteEngine::glSpriteEngine (char *texFile, FileOpener *files, int rows, int cols, int texRows, int texCols)
    :   glTextGrid (texFile, files, rows, cols, texRows, texCols) {
    SetDefaults ();
}
...
void glSpriteEngine::InternalDraw(byte flags) {

    // Draw sprites behind text
    if (flags & DRAW_SPRITES_BEHIND)
        DrawSprites (false);

    // Draw text
    glTextGrid::InternalDraw(flags);

    // Draw sprites in front of text
    if (flags & DRAW_SPRITES_INFRONT)
        DrawSprites (true);
}
