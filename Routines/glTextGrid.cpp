...
glTextGrid::glTextGrid (char *texFile, FileOpener *files, int rows, int cols, int texRows, int texCols)
...
void glTextGrid::RestoreGLState () {
...
void glTextGrid::InternalDraw(byte flags) {
...
void glTextGrid::Draw(byte flags) {
    SaveGLState ();
    InternalDraw (flags);
    RestoreGLState ();
}
...
glTextGridWindow::glTextGridWindow (    bool fullScreen,
                                        bool border,
        ...
        :   glWindow (fullScreen, border, width, height, bpp, stencil, title, allowResizing, fitToWorkArea, resetGLMode) {
    m_textGrid = NULL;
}

void glTextGridWindow::RecreateGLContext () {
    glWindow::RecreateGLContext ();

    if (m_textGrid != NULL)
        m_textGrid->UploadCharsetTexture ();
}
