...
public:
    ...
    // Getters and setters.
    int FrameCount ()                { return m_textures.size (); }
    vmReal Frame ()                  { return m_frame; }
...
class glSpriteEngine : public glTextGrid {
    ...
    virtual void InternalDraw(byte flags);
public:
    vmReal m_camX, m_camY, m_camZ;
    vmReal m_camAngle;

    glSpriteEngine (char *texFile, FileOpener *files, int rows, int cols, int texRows, int texCols);
    void SetDefaults ();
    ...
    vmReal Width ()                  { return m_width; }
    vmReal Height ()                 { return m_height; }
    vmReal FOV ()                    { return m_fov; }
